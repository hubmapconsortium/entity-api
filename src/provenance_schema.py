...
def get_data_access_level(property_key, normalized_class, data_dict):
    ...
    if normalized_class == 'Dataset':
        ...
        count = neo4j_queries.count_attached_published_datasets(app.get_neo4j_db(), normalized_class, data_dict['uuid'])
...
def set_user_sub(property_key, normalized_class, data_dict):
    if 'sub' not in data_dict:
        app.internal_server_error("Missing 'sub' key in 'data_dict' during calling 'set_user_sub()' trigger method.")
...
def set_user_email(property_key, normalized_class, data_dict):
    if 'email' not in data_dict:
        app.internal_server_error("Missing 'email' key in 'data_dict' during calling 'set_user_email()' trigger method.")
...
def set_user_displayname(property_key, normalized_class, data_dict):
    if 'name' not in data_dict:
        app.internal_server_error("Missing 'name' key in 'data_dict' during calling 'set_user_displayname()' trigger method.")
...
def set_uuid(property_key, normalized_class, data_dict):
    if 'uuid' not in data_dict:
        app.internal_server_error("Missing 'uuid' key in 'data_dict' during calling 'set_uuid()' trigger method.")
...
def set_hubmap_id(property_key, normalized_class, data_dict):
    if 'hubmap_id' not in data_dict:
        app.internal_server_error("Missing 'hubmap_id' key in 'data_dict' during calling 'set_hubmap_id()' trigger method.")
...
def get_collection_datasets(property_key, normalized_class, data_dict):
    if 'uuid' not in data_dict:
        app.internal_server_error("Missing 'uuid' key in 'data_dict' during calling 'get_collection_datasets()' trigger method.")
...
def get_dataset_source_uuids(property_key, normalized_class, data_dict):
    if 'uuid' not in data_dict:
        app.internal_server_error("Missing 'uuid' key in 'data_dict' during calling 'get_dataset_source_uuids()' trigger method.")
    return neo4j_queries.get_dataset_source_uuids(app.get_neo4j_db(), data_dict['uuid'])
...
def get_sample_source_uuid(property_key, normalized_class, data_dict):
    if 'uuid' not in data_dict:
        app.internal_server_error("Missing 'uuid' key in 'data_dict' during calling 'get_sample_source_uuid()' trigger method.")
...
def set_activity_creation_action(property_key, normalized_class, data_dict):
    if 'normalized_entity_class' not in data_dict:
        app.internal_server_error("Missing 'normalized_entity_class' key in 'data_dict' during calling 'set_activity_creation_action()' trigger method.")
...
def generate_triggered_data(trigger_type, schema_section, normalized_class, data_dict):
    ...
    for key in class_property_keys:
        if trigger_type in properties[key]:
            ...
            # Call the target trigger method of provenance_schema.py module
            trigger_method_to_call = getattr(provenance_schema, trigger_method_name)
