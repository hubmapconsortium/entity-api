...
def update_entity(id):
    ...
    logger.debug("======update entity with json_list_str======")
    ...
    # We'll need to return all the properties including those 
    # generated by `on_read_trigger` to have a complete result
    complete_dict = schema_manager.get_complete_entity_result(updated_entity_dict)

    # Will also filter the result based on schema
    final_dict = schema_manager.normalize_entity_result_for_response(complete_dict)
...
def create_collection(normalized_entity_class, json_data_dict):
    ...
    logger.debug("======create_collection() with escaped_json_list_str======")
...
def create_donor(normalized_entity_class, json_data_dict):
    ...
    logger.debug("======create_donor() with escaped_json_list_str======")
    logger.debug(escaped_json_list_str)
...
def create_dataset(normalized_entity_class, json_data_dict):
    ...
    logger.debug("======create_dataset() with escaped_json_list_str======")
    logger.debug(escaped_json_list_str)
