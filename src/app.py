...
def get_entity_provenance(id):
    ...
    logger.debug("===========raw_provenance_dict===========")
    logger.debug(raw_provenance_dict)
    ...
    for node_dict in raw_provenance_dict['nodes']:
        if (node_dict['label'] == 'Entity') and (node_dict['entity_type'] != 'Lab'):
            # We'll need to return all the properties including those 
            # generated by `on_read_trigger` to have a complete result
            # Note: DON't call schema_manager.normalize_entity_result_for_response()
            # that'll remove properties like 'label' since it's not in the schema
            complete_dict = schema_manager.get_complete_entity_result(node_dict)
            normalized_provenance_dict['nodes'].append(complete_dict)
        else:
            # No need to normalize Activity nodes and Entity Lab nodes
