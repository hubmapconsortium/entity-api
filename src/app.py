...
def get_entity_by_id(id):
    ...
    entity_scope = _get_entity_visibility(normalized_entity_type=normalized_entity_type, entity_dict=complete_dict)
    entity_scope_is_public = False
    if entity_scope == DataVisibilityEnum.PUBLIC:
        entity_scope_is_public = True
    ...
    if entity_scope_is_public is False and isinstance(user_token, Response):
    ...
    user_is_in_hubmap_read_group = user_in_hubmap_read_group(request)
    ...
    if not entity_scope_is_public and not user_is_in_hubmap_read_group:
    ...
    if bool(request.args):
        ...
        if property_key is not None:
            ...
            if entity_scope_is_public and not user_is_in_hubmap_read_group:
                complete_dict = schema_manager.exclude_properties_from_response(fields_to_exclude, complete_dict)
        ...
        if entity_scope and not user_is_in_hubmap_read_group:
...
def _get_metadata_by_id(entity_id:str=None, metadata_scope:MetadataScopeEnum=MetadataScopeEnum.INDEX):
    ...
    if bool(request.args):
        ...
        # Response with the dict
        if public_scope and has_access is False:
