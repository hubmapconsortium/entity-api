...
def get_collections():
    ...
    try:
        ...
        if component == 'all':
            ...
            query = "MATCH (collection:Collection)<-[:IN_COLLECTION]-(dataset:Entity)-[:HAS_METADATA]->(metadata:Metadata {{provenance_group_uuid: '{guuid}'}}) " + where_clause + "RETURN collection.uuid, collection.label, collection.description, collection.creators, collection.doi_registered, collection.display_doi, collection.registered_doi, collection.doi_url, collection.provenance_create_timestamp, collection.provenance_modified_timestamp, collection.provenance_user_displayname, apoc.coll.toSet(COLLECT(dataset.uuid)) AS dataset_uuid_list"
