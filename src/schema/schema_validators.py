...
def _is_entity_locked_against_update(existing_entity_dict):
    ...
    if entity_type in ['Publication','Dataset']:
        if 'status' in existing_entity_dict and existing_entity_dict['status'] == 'Published':
            raise schema_errors.SchemaValidationException(f"{entity_type} cannot be modified, due to"
        ...
        raise schema_errors.SchemaValidationException(f'Unable to check if {entity_type} for {entity_uuid} is locked!')
